...
export function ZoneParameterForm({
  ...
  const maxFloors = series.floorMax;
  const [area, setArea] = useState(60);
  const [floor, setFloor] = useState(Math.min(3, maxFloors));
  const [totalFloors, setTotalFloors] = useState(maxFloors);
    ...
    <div className="fade-enter">
      ...
      <div className="mb-6">
        <div className="flex justify-between mb-2">
          ...
          <span className="text-sm font-bold text-[#C8A44E] font-mono">{floor} из {totalFloors}</span>
        </div>
        <input type="range" min={1} max={totalFloors} value={floor} onChange={(e) => setFloor(Number(e.target.value))} className="w-full" />
        <div className="flex justify-between text-xs text-[#5A6478] mt-1">
          <span>1 этаж</span>
          <span>{totalFloors} этаж</span>
        </div>
      </div>
      ...
      <div className="mb-6">
        <div className="flex justify-between mb-2">
          <span className="text-xs font-medium text-[#5A6478] uppercase tracking-[0.15em]">Этажность дома</span>
          <span className="text-sm font-bold text-[#C8A44E] font-mono">{totalFloors} эт.</span>
        </div>
        <input
          type="range"
          min={series.floorMin}
          ...
          onChange={(e) => {
            ...
            if (floor > newTotal) setFloor(newTotal);
          }}
          className="w-full"
        />
        <div className="flex justify-between text-xs text-[#5A6478] mt-1">
          <span>{series.floorMin} эт.</span>
          <span>{series.floorMax} эт.</span>
        </div>
      </div>
          ...
          {VIEW_OPTIONS.map((opt) => (
            <button
              ...
              style={{
                ...
                background: view === opt.value ? "rgba(200,164,78,0.06)" : "rgba(255,255,255,0.02)",
                boxShadow: view === opt.value ? "0 0 20px rgba(200,164,78,0.08)" : "none",
              }}
            >
          ...
          {CONDITION_OPTIONS.map((opt) => (
            <button
              ...
              className={`rounded-xl px-4 py-3.5 text-left text-sm font-medium transition-all duration-300 cursor-pointer ${
                condition === opt.value ? "text-white" : "text-[#7A8299] hover:border-[rgba(255,255,255,0.12)]"
              }`}
              style={{
                ...
                background: condition === opt.value ? "rgba(200,164,78,0.04)" : "rgba(255,255,255,0.02)",
              }}
            >
              {opt.label}
            </button>
      ...
      <Button
        variant="primary"
        onClick={() => onSubmit({ area, floor, totalFloors, view, condition })}
