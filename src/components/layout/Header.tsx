  ...
  return (
    <header
      className={`header-animate fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-[rgba(255,255,255,0.85)] backdrop-blur-[20px] backdrop-saturate-[180%] border-b border-[rgba(0,0,0,0.06)]"
          : "bg-transparent"
      }`}
    ...
    >
      ...
      {menuOpen && (
        <div className="lg:hidden fixed inset-0 top-[56px] sm:top-[64px] bg-[rgba(255,255,255,0.97)] backdrop-blur-xl z-40">
          <nav className="flex flex-col items-center justify-center h-full gap-6">
            {NAV_LINKS.map((link, i) => (
              <a
                ...
                className="text-2xl font-medium text-[#6B7280] hover:text-[#1A2332] transition-colors"
                style={{
            ...
            <a
              href="#contacts"
              onClick={() => setMenuOpen(false)}
              className="mt-4 rounded-full bg-gradient-to-r from-[#66BB6A] to-[#26A69A] px-8 py-3.5 text-[15px] font-semibold text-white"
              style={{
